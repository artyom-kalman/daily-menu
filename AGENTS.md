...
- Telegram UX: two inline buttons — `today_menu` and morning `Присылать утром` / `Отписаться`. Opt-in/out is a toast + button flip, not a new message. No student commands. Inbound group / channel updates are ignored. Weekday before 12:30 KST with no today's `menus` rows: «Меню ещё не выложили. Попробуйте позже.» (no on-demand scrape; cron fetches).

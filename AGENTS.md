...
- Telegram UX: two inline buttons — `today_menu` and morning `Присылать утром` / `Отписаться`. No student commands. Inbound group / channel updates are ignored.
...
- Morning push: after a weekday cron fetch with at least one complete live tray (not a stub or closed notice). One message per opted-in chat. Optional channel post to `TELEGRAM_CHANNEL_CHAT_ID` (same gate, once per day, no keyboard). Store `chatId` in Convex only; never send it to Aptabase.

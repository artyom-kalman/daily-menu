...
- Telegram UX: first DM with no `chatPrefs.locale` is a language picker (`Language / Язык`, Русский / English). After a locale is stored, any student DM sends today's menu. Inline buttons: `today_menu`, morning opt-in/out (toast + button flip), Peony / Azilea hall toggles (toast + resend; default both; last hall stays on), and Change language (picker on that message, then resend menu). No student commands. Do not infer Telegram `language_code`. Channel posts stay Russian, both halls, no keyboard. Inbound group / channel updates are ignored. Weekday before 12:30 KST with no today's `menus` rows: localized not-ready copy (no on-demand scrape; cron fetches).
- Admin commands (`/status`, `/refetch`, `/stats`) only for `ADMIN_CHAT_ID`; everyone else gets today's menu with the locale keyboard. `/stats` sends `APTABASE_DASHBOARD_URL`
...
- Morning push: after a weekday cron fetch when **both** cafeterias are settled (each has its own complete live tray of 5+ dishes, or a closed/holiday notice). Do not push while either hall is still a stub, except on the last 12:30 KST attempt, which sends whatever is posted even if one hall is empty. Skip weekends and both-empty days. Completeness is campus-wide even if a chat hid a hall. One message per opted-in chat, formatted in that chat's `chatPrefs.locale` and `chatPrefs.halls` (missing row → insert `ru` and send both halls in Russian). Optional channel post to `TELEGRAM_CHANNEL_CHAT_ID` (same gate, once per day, Russian, both halls, no keyboard). Store `chatId` in Convex only; never send it to Aptabase. Dish gloss is `gloss[locale]` on each dish; UI chrome lives in `i18n.ts`.
- Product events go to Aptabase (`start`, `today_menu`, `channel_post`, `scrape_ok` / `scrape_empty` / `scrape_error`). Optional `APTABASE_APP_KEY`; no-op if unset. Do not send `chatId` or menu text. Convex **dev** (`enchanted-goshawk-667`) uses Aptabase Debug; prod uses Release.

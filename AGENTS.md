...
- Telegram UX: two inline buttons — `today_menu` and morning `Присылать утром` / `Отписаться`. No student commands. Weekday before 12:30 KST with no today's `menus` rows: «Меню ещё не выложили. Попробуйте позже.» (no on-demand scrape; cron fetches).

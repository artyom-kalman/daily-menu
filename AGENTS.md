...
- Telegram UX: first DM with no `chatPrefs.locale` is a language picker (`Language / Язык`, Русский / English). After a locale is stored, any student DM sends today's menu. Inline buttons: `today_menu`, morning opt-in/out (toast + button flip), and Change language (picker on that message, then resend menu). No student commands. Do not infer Telegram `language_code`. Channel posts stay Russian, no keyboard. Inbound group / channel updates are ignored. Weekday before 12:30 KST with no today's `menus` rows: localized not-ready copy (no on-demand scrape; cron fetches).

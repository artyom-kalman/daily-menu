...
convex/
  ...
  chatPrefs.ts         per-chat locale and hall preference
...
1. First private message with no stored locale → language picker (`Language / Язык`, **Русский** / **English**). After a locale is stored, any message (e.g. `/start`) → today's menu with **Сегодняшнее меню** / **Today's menu**, morning opt-in, Peony / Azilea hall toggles (default both), and **Сменить язык** / **Change language**. If it is a weekday before 12:30 KST and today's menus are still missing, the reply is the localized not-ready copy instead of scraping on that request.
2. User taps **Сегодняшнее меню** / **Today's menu** → the same formatted menu again (refresh, including stub trays), except on a weekday before 12:30 KST with no today's rows: that tap also sends the not-ready copy and does not scrape. A KST date line (`9 сен · 09:14` / `9 Sep · 09:14`) sits above the first visible hall; incomplete live trays add italic still-updating copy under that hall until 12:30 KST. Russian hall lines keep `Peony · верхняя` / `Azilea · нижняя`; English uses `피오니 · 지운관` / `아질리아 · 창조관`. Hangul names stay; gloss comes from `gloss[locale]`. Visible halls are grouped by tray slot. Telegram HTML: bold names, italic gloss and section labels, compact chili. Staples bunch on one line. The buttons stay on the menu.
3. **Присылать утром** / **Send in the morning** stores that `chatId` in Convex and toasts confirmation on the same message (the button flips to **Отписаться** / **Unsubscribe**; no extra chat bubble). After a weekday scrape when **both** halls are settled, opted-in chats get the menu once in that chat's locale and hall filter. A missing `chatPrefs` row is created as `ru` so existing subscribers still get Russian, both halls. Hall toggles toast and resend; at least one hall stays on. **Change language** edits the keyboard into Русский / English, then resends the menu. The channel post stays Russian, both halls, with no keyboard.
...
- Tapping **Сегодняшнее меню** re-fetches when there is still no complete live menu (empty, stub, or `no_info`), except on a weekday before **12:30 KST** with **no** today's rows: that path sends «Меню ещё не выложили. Попробуйте позже.» and leaves fetching to cron.
- After a weekday fetch when **both** halls are settled (each 5+ live dishes, or a closed notice), opted-in chats get that menu once (`subscribers.lastPushedDate`). A stub or empty hall waits. On the last 12:30 KST attempt the bot sends anyway even if one side is still empty (`Нет информации`). If `TELEGRAM_CHANNEL_CHAT_ID` is set, the same text is posted once to that channel (`channelPush.lastPostedDate`), with no keyboard. Weekends and both-empty days are skipped. A failed send does not stop the rest of the batch; a blocked DM chat is dropped. A blocked channel does not mark the day posted (cron retries can try again).
